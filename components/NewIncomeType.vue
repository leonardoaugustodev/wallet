...
export default {
    ...
    computed: {
        formTitle() {
            return !this.record._id ? this.$t('newIncomeType') : this.$t('editIncomeType')
        },
    },
    watch: {
        dialog(val) {
    ...
    methods: {
        async save() {
            ...
            await this.$store.dispatch('getAllIncomeTypes')
            this.close()
        },
        async create(record) {
            try {
                const ref = this.$fire.firestore.collection('incomeTypes').doc()
        ...
        async update(record) {
            const ref = this.$fire.firestore.collection('incomeTypes').doc(record._id)
            try {
                record._updatedAt = this.$fireModule.firestore.FieldValue.serverTimestamp()
        ...
        deleteItem() {
            ...
            this.$store.dispatch('deleteIncomeType', this.editedItem._id)
            this.close()
        },
        close() {
            this.dialog = false
